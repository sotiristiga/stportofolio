...
with kpi1:
    st.markdown(lnk + metrics_customize(0,204,102,"fas fa-users","Πελάτες",All2['id'].nunique()), unsafe_allow_html=True)

with kpi2:
    st.markdown(lnk + metrics_customize(0,204,102,"fas fa-file-contract","Συμβόλαια",All2['N_Policy'].nunique()), unsafe_allow_html=True)

with kpi3:
    st.markdown(lnk + metrics_customize(0,204,102,"fas fa-euro-sign","Καθαρά Ασφάλιστρα",All2['Net'].sum().round(2)), unsafe_allow_html=True)
with kpi4:
    st.markdown(lnk + metrics_customize(0,204,102,"fas fa-euro-sign","Προμήθειες",All2['Commissions'].sum().round(2)), unsafe_allow_html=True)

with kpi5:
    st.markdown(lnk + metrics_customize(0,204,102,"fas fa-percent","Ποσοστό Προμήθειας",((All2['Commissions'].sum()/All2['Net'].sum()).round(4)*100)), unsafe_allow_html=True)
...
tab1,tab2,tab3,tab4 = st.tabs(["Εξέλιξη Παραγωγής","Κλάδος ασφάλισης",'Δημογραφικά Πελατών','Διάρκειες Συμβολαίων'])
with tab1:
...
with tab2:
    ...
    with col2:
        ...
        fig_barplot.update_traces(textfont_size=17, texttemplate = '%{text:.3s} €',textangle=0, textposition="inside", cliponaxis=False)
    ...
    with tabs21:
        ...
        fig_line_polcou = px.line(cat_sum_year, 
                                    ...
                                    color_discrete_sequence= px.colors.sequential.Aggrnyl,
                                    labels={'count':'Σύνολο συμβολαίων','Year':'Έτος',"Category":"Κλάδος"},markers=True)
        fig_line_polcou.update_layout(plot_bgcolor='white',font_size=13)
        st.write(fig_line_polcou)
    ...
    with tabs23:
        fig_line_polcou = px.line(cat_sum_year, 
                                    ...
                                    title='Προμήθειες',
                                    color_discrete_sequence= px.colors.sequential.Aggrnyl,
                                    labels={'Commissions':'Προμήθειες','Year':'Έτος',"Category":"Κλάδος"},markers=True)
        fig_line_polcou.update_layout(plot_bgcolor='white',font_size=13)
        st.write(fig_line_polcou)
    ...
    cat_mean_month_com=px.bar(cat_dur_mean_month[cat_dur_mean_month['Category']==option1],x='Month',y='count',title='',
                    labels={'count':'Συνολικά συμβόλαια','Category':'Κλάδος','Month':'Μήνας',"Duration_gr":'Διάρκεια Συμβολαίων'},color='Duration_gr',
